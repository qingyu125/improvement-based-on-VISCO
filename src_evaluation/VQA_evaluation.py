...
def post_process(prediction, stem=True):
    """
...
def VQAEvaluate(answer, label, meta_data):
    if meta_data["src_dataset"] == "TextVQA":
        try:
            processed_answer = post_process(answer)
            correct_count = 0
            for l in label:
                processed_label = post_process(l)
                if processed_answer == processed_label:
                    correct_count += 1
            if correct_count >= 3:
                return True
        except:
